...
import com.nimbits.client.ui.controls.ReportHelper;
...
//            final String data = model.get(Const.PARAM_DATA);
            final Value value = ValueModel.getInstance(SimpleValue.getInstance(v), timestamp);

            valueService.recordValueRpc(model.getBaseEntity(), value, new SaveValueAsyncCallback(model));
            model.setDirty(false);
        }
    ...
    private static class MoveEntityAsyncCallback implements AsyncCallback<List<Entity>> {
    ...
    private class DNDListener extends com.extjs.gxt.ui.client.event.DNDListener {
        ...
        private void moveEntity(final Entity draggedEntity, final Entity target) {
            ...
            if (target.getOwner().equals(draggedEntity.getOwner())) {
                ...
                entityService.addUpdateEntityRpc(Arrays.asList(draggedEntity), new MoveEntityAsyncCallback());
            }
        }
        ...
        public void handleEvent(final GridEvent be) {
            ...
            if (be.getColIndex() == valueColumnIndex) { //only save when the value is updated
                ...
                if (!model.isReadOnly()) {
                    ...
                    value = ValueModel.getInstance(SimpleValue.getInstance(valueAndNote), timestamp);
                    valueService.recordValueRpc(entity, value, new RecordValueCallback(be, model));
