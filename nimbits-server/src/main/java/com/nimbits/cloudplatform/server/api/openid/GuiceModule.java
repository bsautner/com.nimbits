...
import com.nimbits.cloudplatform.server.api.openid.appengine.AppEngineHttpFetcher;
import com.nimbits.cloudplatform.server.api.openid.appengine.AppEngineTrustsRootProvider;
import com.nimbits.cloudplatform.server.api.openid.appengine.Openid4javaFetcher;
...
public class GuiceModule extends AbstractModule {
    ...
    protected void configure() {
        ...
        bind(ConsumerAssociationStore.class).toInstance(associationStore);
    ...
    public static class AppEngineModule extends AbstractModule {
        @Override
        protected void configure() {
            bind(HttpFetcher.class).to(AppEngineHttpFetcher.class).in(
                    Scopes.SINGLETON);
            bind(TrustRootsProvider.class)
                    .to(AppEngineTrustsRootProvider.class).in(Scopes.SINGLETON);
            bind(org.openid4java.util.HttpFetcher.class).to(
                    Openid4javaFetcher.class).in(Scopes.SINGLETON);
        }
    }
