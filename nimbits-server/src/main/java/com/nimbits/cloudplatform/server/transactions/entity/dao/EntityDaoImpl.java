...
package com.nimbits.cloudplatform.server.transactions.entity.dao;
...
public class EntityDaoImpl implements EntityDao {
    ...
    public EntityDaoImpl(PersistenceManagerFactory pmf) {
        this.pmf = pmf;
    }
        ...
        if (! sample.isEmpty()) {
            ...
            if (!commit.getEntityType().equals(EntityType.user)) {
                ...
                if (commit.getEntityType().isTrigger()) {
                    new RecursionValidation().validate(user, (Trigger) commit);
                }
    ...
    @Override
    public  List<Entity> getEntitiesBySource(final User user, final Entity source, final Class<?> cls) throws Exception {
        final PersistenceManager pm = pmf.getPersistenceManager();
        try {
    ...
    @Override
    public Map<String, Entity> getSystemWideEntityMap(User admin, final EntityType type) throws Exception {
