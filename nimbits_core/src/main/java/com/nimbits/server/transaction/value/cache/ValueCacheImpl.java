...
public class ValueCacheImpl implements ValueCache {
    ...
    public ValueCacheImpl(NimbitsEngine engine, TaskService taskService) {
        this.valueDAO = ValueServiceFactory.getDaoInstance(engine);
    ...
    public List<Value> getValueBuffer(final Entity entity) {
        if (cache.containsKey(bufferCacheKey(entity))) {
            ...
            Object buffer = cache.get(bufferCacheKey(entity));
            if (buffer != null) {
                return (List<Value>) buffer;
    ...
    public Value recordValue(final User user,
        ...
        if (buffer != null && buffer.size() > Const.CONST_MAX_CACHED_VALUE_SIZE) {
            taskService.startMoveCachedValuesToStoreTask(entity);
        }
    ...
    public List<Value> getTopDataSeries(final Entity entity, final int maxValues, final Date endDate)  {
        ...
        if (cached != null && cached.size() > maxValues) {
            ...
            final List<Value> stored = engine.getBlobStore().getTopDataSeries(entity, maxValues, endDate);
    ...
    @Override
    public List<Value> getDataSegment(final Entity entity, final Range<Date> timespan, final Range<Integer> range)  {
        final List<Value> stored = engine.getBlobStore().getDataSegment(entity, timespan);
        final List<Value> cached = getValueBuffer(entity);
        List<Value> allData =  mergeAndSort(stored, cached, timespan);   //todo cache result
        List<Value> result = new ArrayList<Value>();
        int max = range.upperEndpoint() > allData.size() ? allData.size() : range.upperEndpoint();
        for (int i = range.lowerEndpoint(); i < max; i++) {
    ...
    @Override
    public List<Value> getSeries(final Entity entity, final Range<Date> timespan)  {
        final List<Value> stored = engine.getBlobStore().getDataSegment(entity, timespan);
        final List<Value> cached = getValueBuffer(entity);
        List<Value> allData =  mergeAndSort(stored, cached, timespan);   //todo cache result
    ...
    public List<ValueBlobStore> getAllStores(final Entity entity)  {
        return engine.getBlobStore().getAllStores(entity);
    }


    @Override
    public void deleteExpiredData(final Entity entity) {
        engine.getBlobStore().deleteExpiredData(entity);
    }
...
//        List<Value> stored = getDataSegment(entity, timespan, Range.closed(0, SEC));
...
//    }
    ...
    @Override
    public void consolidateBlobs(Point entity) {
        valueDAO.consolidateBlobs(entity);
    }
    ...
    private <E> List<E> mergeAndSort(final Collection<E> first, final Collection<E> second, final Range<Date> timespan) {
        first.addAll(second);
        ...
        for (final Map.Entry<?, E> longObjectEntry : sorted_map.entrySet()) {
            ...
            //if (l >= timespan.lowerEndpoint() - SEC && l <= timespan.upperEndpoint() + SEC) {
            if (timespan.contains(new Date(l))) {
