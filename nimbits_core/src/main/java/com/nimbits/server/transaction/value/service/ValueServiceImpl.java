...
public class ValueServiceImpl implements ValueService {
    ...
    public ValueServiceImpl(NimbitsEngine engine, TaskService taskService) {
        this.cacheService = ValueServiceFactory.getCacheInstance(engine, taskService);
    ...
    @Override
    public boolean checkIdle(final User user, final Point p)  {
        final Calendar c = Calendar.getInstance();
        c.add(Calendar.SECOND, p.getIdleSeconds() * -1);
        boolean retVal = false;

        //
        final List<Entity> result = entityService.getEntityByKey(user,
        ...
        if (!result.isEmpty()) {
            ...
            if (p.getIdleSeconds() > 0 && !v.isEmpty() &&
                ...
                entityService.addUpdateEntity(u, Arrays.asList((Entity) p));
                // PointServiceFactory.getInstance().updatePoint(u, p);
                final Value va = ValueFactory.createValueModel(v.get(0), AlertType.IdleAlert);
                SubscriptionServiceFactory.getServiceInstance(engine, taskService).processSubscriptions(u, p, va);
                retVal = true;
            }
        }
        return retVal;
    ...
    public Value recordValue(final User user,
        ...
        if (! ignoreByAuthLevel(user, point)) {
            ...
            if (!ignoredByDate && !ignoredByCompression) {
                ...
                taskService.startRecordValueTask(user, point, v);
                return retObj;
            } else {
...
//    }
    ...
    @Override
    public List<Value> getSeries(Entity entity, Range<Date> timespanRange) {
...
//                            && newValue >= min;
    ...
    public List<Value> getDataSegment(final Entity entity, final Range<Date> timespan, final Range<Integer> range)   {
        return cacheService.getDataSegment(entity, timespan, range);
    }

    @Override
    public List<Value> getDataSegment(final Entity entity, final Range<Date> timespan)   {
        return cacheService.getDataSegment(entity, timespan, Range.closed(0, 1000));
    }
    ...
    public void consolidateBlobs(Point entity) {
        cacheService.consolidateBlobs(entity);
    }
