...
/**
 * Topic types can be set in the topic's property menu on the nimbits console.
 * <ul>
 * <li>basic - stores values as they come in</li>
 * <li>backend - system topic, do not use</li>
...
public enum TopicType implements Serializable {
    ...
    private static final Map<Integer, TopicType> lookup = new HashMap<Integer, TopicType>(TopicType.values().length);

    static {
        for (TopicType s : EnumSet.allOf(TopicType.class))
            lookup.put(s.code, s);
    }
    ...
    public static TopicType get(int code) {
